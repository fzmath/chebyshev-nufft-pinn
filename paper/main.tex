...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive numerical validation}: We validate the method on 1D and 2D Poisson equations with Dirichlet boundary conditions, comparing with AD-PINN, pure Chebyshev PINN, and pure finufft PINN. Ablation studies isolate the contributions of the boundary layer width, overlap width, and interior basis selection. The experiments demonstrate that the hybrid layer achieves accuracy comparable to AD-PINN while training significantly faster, and that both pure Chebyshev and pure Fourier methods fail on non-uniform non-periodic problems.
\end{enumerate}
...
\end{theorem}
\begin{proof}
The error decomposes by region:
\begin{enumerate}
    \item In $\Omega_L \cup \Omega_R$, the Chebyshev derivative error is bounded by Proposition~\ref{prop:cheb_conv}, giving the $C_1 \rho^{-N_c} N_c^2$ term. The direct polynomial evaluation of Eq.~\eqref{eq:cheb_direct_eval} avoids a second interpolation step, so only one scattered regression error contributes.
    \item In $\Omega_I$, the error depends on the interior basis: Chebyshev gives exponential convergence; NUFFT gives the bound of Proposition~\ref{prop:nufft_acc}, which degrades algebraically for non-periodic functions due to the discontinuity at the period boundary.
    \item The scattered regression from non-uniform points to Chebyshev coefficients contributes $C_3 \epsilon_{\mathrm{reg}}$, which is controlled by the SVD truncation threshold and the condition number of the Vandermonde matrix; for well-distributed points and analytic functions, the regression error decays exponentially with $N_c$.
    \item In the overlap region, the Schwarz-style extension ensures both representations remain within their reference domains, eliminating extrapolation error. The smoothstep window blends continuously, with consistency error bounded by the difference between the two approximations, which vanishes as $N_c \to \infty$.
\end{enumerate}
Taking the maximum over all regions yields the bound.
\end{proof}
...
\begin{equation}
    ...
    + \lambda_{\mathrm{bc}} \frac{1}{M_b} \sum_{j=1}^{M_b} |\calB[u_\theta](\bfx_j^b) - g(\bfx_j^b)|^2
    + \lambda_{\mathrm{data}} \frac{1}{M_d} \sum_{j=1}^{M_d} |u_\theta(\bfx_j^d) - u^*(\bfx_j^d)|^2,
    \label{eq:pinn_loss}
\end{equation}
...
Table~\ref{tab:poisson1d} summarizes the relative $L^2$ errors and training times. The hybrid layer achieves accuracy comparable to AD-PINN ($4.78\times10^{-6}$ vs $4.72\times10^{-6}$) while training $2.5\times$ faster (41.4\,s vs 104.6\,s per seed). The pure Chebyshev method also converges in 1D (the boundary layer interpolation is sufficient for a single interval), but the pure finufft method completely fails ($L^2\approx14$) because the periodic Fourier basis cannot represent the non-periodic solution, leading to severe Gibbs phenomenon at the boundaries.
...
\subsubsection{Setup}
We use $M=5000$ Halton-sampled collocation points, a 4-hidden-layer MLP (128 neurons, Tanh activation), and train for 8000 steps with Adam (lr=$10^{-3}$, warmup 1000, cosine annealing). Dirichlet BCs are enforced via the hard constraint $u_\theta(x,y) = x(1-x)y(1-y)\,\mathrm{MLP}_\theta(x,y)$. The hybrid layer is applied tensor-product-wise along each spatial dimension. Three random seeds are used.

\subsubsection{Results}
Table~\ref{tab:poisson2d} summarizes the results. The key observations are:

\begin{enumerate}
    \item \textbf{Hybrid layer converges in 2D}: The hybrid method achieves $L^2 = 2.07\times10^{-4}$, demonstrating that the domain-decomposition approach scales to higher dimensions.
    \item \textbf{Speed advantage}: The hybrid layer trains $3.1\times$ faster than AD-PINN (521\,s vs 1636\,s per seed), because the spatial derivatives are computed via a pre-assembled constant matrix rather than per-sample autograd.
    \item \textbf{Pure Chebyshev fails in 2D non-uniform}: The pure Chebyshev method diverges ($L^2\approx1.48$) because interpolating 2D non-uniform data onto a Chebyshev tensor grid introduces large errors that propagate through the second derivative. This confirms that the hybrid's SVD-based scattered regression and domain decomposition are essential.
    \item \textbf{Pure finufft fails on non-periodic domains}: The periodic NUFFT method also fails ($L^2\approx0.77$) due to Gibbs phenomenon at the boundaries, consistent with the 1D result.
\end{enumerate}

\begin{table}[t]
\centering
\caption{2D Poisson equation: relative $L^2$ error and training time (mean$\pm$std over 3 seeds).}
\label{tab:poisson2d}
\begin{tabular}{lcc}
\toprule
Method & Rel.\ $L^2$ error & Time (s/seed) \\
...
\textbf{Hybrid-PINN (Ours)} & $\mathbf{2.07\times10^{-4} \pm 1.36\times10^{-4}}$ & \textbf{521.3} \\
...
We note that AD-PINN achieves higher accuracy in 2D ($1.41\times10^{-5}$ vs $2.07\times10^{-4}$). This gap is attributable to the SVD regression error in the hybrid layer's boundary layers, which becomes more pronounced in 2D due to the tensor-product interpolation. Increasing $N_c$ or using a more sophisticated scattered interpolation (e.g., RBF with optimized shape parameter) can narrow this gap, at the cost of preprocessing time. The hybrid layer's primary advantage is computational speed for problems where moderate accuracy is sufficient or where AD becomes prohibitively expensive (e.g., high-dimensional or large-scale problems).
...
\centering
\begin{subfigure}{0.48\textwidth}
\includegraphics[width=\textwidth]{../figures/poisson_2d_convergence.png}
\caption{Training convergence}
...
\end{subfigure}
\caption{2D Poisson equation: (a) relative $L^2$ error vs training step, (b) predicted solution (top) and exact solution (bottom).}
\label{fig:poisson2d}
\end{figure}
...
The most important ablation compares the full hybrid layer against its two pure components on non-uniform non-periodic problems. The results (already reported in Tables~\ref{tab:poisson1d} and~\ref{tab:poisson2d}) demonstrate that:
\begin{itemize}
    \item In 1D, pure Chebyshev converges (because a single interval interpolation is sufficient), but pure NUFFT diverges ($L^2\approx14$) due to Gibbs phenomenon at the non-periodic boundaries.
    \item In 2D, both pure Chebyshev ($L^2\approx1.48$) and pure NUFFT ($L^2\approx0.77$) diverge, because 2D tensor-product interpolation of scattered data introduces large errors that propagate through the second derivative.
    \item The hybrid layer converges in both 1D and 2D, confirming that the domain decomposition with SVD-based scattered regression and Schwarz overlap is essential for this problem class.
\end{itemize}
...
Table~\ref{tab:neumann} summarizes the results. The Neumann problem is more challenging than Dirichlet because the boundary condition involves derivatives rather than function values, and the solution is defined only up to an additive constant (a zero-energy mode in the Laplacian). Both methods achieve moderate accuracy: AD-PINN reaches $L^2 = 0.253 \pm 0.049$, while the hybrid layer reaches $L^2 = 0.160 \pm 0.087$, with the hybrid layer training 1.8$\times$ faster (163\,s vs 300\,s). The hybrid layer's spectral derivative provides more accurate boundary derivative evaluation, which helps enforce the Neumann soft constraint more precisely. We note that both methods could be improved by adding a mean-zero constraint to remove the constant nullspace, or by using a larger boundary loss weight $\lambda_{\mathrm{bc}}$.

\begin{table}[t]
\centering
\caption{1D Poisson with Neumann boundary conditions ($u'(0)=u'(1)=0$, exact $u=\cos(\pi x)$), 3 seeds.}
\label{tab:neumann}
\begin{tabular}{lcc}
\toprule
Method & Rel.\ $L^2$ error & Time (s/seed) \\
\midrule
AD-PINN & $0.253 \pm 0.049$ & 300 \\
\textbf{Hybrid-PINN (Ours)} & $\mathbf{0.160 \pm 0.087}$ & \textbf{163} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item The hybrid layer achieves accuracy comparable to AD-PINN on Dirichlet problems ($4.78\times10^{-6}$ vs $4.72\times10^{-6}$ in 1D) while training $2.5\times$ faster, and $3.1\times$ faster in 2D.
    \item On Neumann and Robin boundary conditions, the hybrid layer matches or outperforms AD-PINN (Robin: $1.46\times10^{-4}$ vs $1.80\times10^{-4}$), with speedups of $1.6$--$1.8\times$.
...
The current limitation is that the hybrid layer's accuracy in 2D is one order below AD-PINN, due to SVD regression error in the boundary layers. Future work includes increasing Chebyshev modes, optimizing the scattered interpolation, extending to 3D and curvilinear domains, and applying the method to parameter inverse problems and more complex nonlinear PDEs.
